...
class BookingService(
     ...
     * 결제 -> 예약 순으로 진행
     */
    fun preCreate(hotelId: Long, roomId: Long, member: Member): BookingFormResponse {
    ...
    fun create(member: Member, bookingRequest: BookingRequest) {
        try {
            ...
            val hotel = hotelRepository.findById(bookingRequest.hotelId)
                .orElseThrow { ErrorCode.HOTEL_NOT_FOUND.throwServiceException() }
            val payment = paymentService.create(bookingRequest)

            // 예약 데이터 생성
            var booking = Booking(room, hotel, member, payment, bookingRequest.checkInDate, bookingRequest.checkOutDate)
    ...
    fun tryGetMyBookings(member: Member, page: Int, pageSize: Int): Page<BookingResponseSummary> {
    ...
    // 예약 상세 조회
    fun tryGetBookingDetails(member: Member, bookingId: Long): BookingResponseDetails {
    ...
    // 예약 완료 처리
    fun trySetCompleted(member: Member, bookingId: Long) {
        val booking = findById(bookingId)
    ...
    @Transactional
    fun setCompleted(booking: Booking) {
        try {
    ...
    fun findByMember(member: Member, page: Int, pageSize: Int): Page<Booking> {
    ...
    fun findByHotel(hotel: Hotel, page: Int, pageSize: Int): Page<Booking> {
        val pageRequest = PageRequest.of(page - 1, pageSize, Sort.by(Sort.Order.desc("id")))
