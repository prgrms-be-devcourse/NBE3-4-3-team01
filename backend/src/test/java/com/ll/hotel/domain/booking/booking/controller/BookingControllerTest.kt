...
class BookingControllerTest {
    ...
    fun t1_1() {
        ...
        // 검증
        resultActions
            .andExpect(handler().handlerType(BookingController::class.java))
    ...
    fun t2_1() {
        ...
        val bookingRequest = BookingRequest(
            ...
            "1234567890", 1004, Instant.now().epochSecond
        )
        val resultActions = mvc
    ...
    fun t4_1() {
        // 인가 설정
        val customer = memberRepository.findByMemberName("business1")
            .orElseThrow { IllegalArgumentException("테스트 데이터가 잘못되었습니다.") }
        setMock(customer)
    ...
    fun t4_2() {
        // 잘못된 인가 설정
        val customer = memberRepository.findByMemberName("customer1") // 비사업자, 호텔 비소유자는 조회 불가
            .orElseThrow { IllegalArgumentException("테스트 데이터가 잘못되었습니다.") }
        setMock(customer)
    ...
    fun t5_2() {
        // 잘못된 인가 설정
        val customer = memberRepository.findByMemberName("customer2") // 예약 비관계자는 조회 불가
            .orElseThrow { IllegalArgumentException("테스트 데이터가 잘못되었습니다.") }
        setMock(customer)
    ...
    fun t6_2() {
        // 잘못된 인가 설정
        val customer = memberRepository.findByMemberName("customer2") // 예약 비관계자는 취소 불가
            .orElseThrow { IllegalArgumentException("테스트 데이터가 잘못되었습니다.") }
        setMock(customer)
    ...
    fun t7_2() {
        // 잘못된 인가 설정
        val customer = memberRepository.findByMemberName("customer1") // 예약을 소유한 사업자만 완료 처리 가능
            .orElseThrow { IllegalArgumentException("테스트 데이터가 잘못되었습니다.") }
        setMock(customer)
